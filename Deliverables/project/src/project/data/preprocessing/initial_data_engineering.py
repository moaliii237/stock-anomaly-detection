...
MINUTE_DATA_CSV_PATH = "data/BKNG_minute_data.csv"
DAILY_DATA_CSV_PATH = "data/BKNG.csv"
OUTPUT_CSV_PATH = "data/BKNG_FINAL.CSV"
...
def calculate_atr(df_slice: pd.DataFrame, window: int = 14) -> pd.Series:
    ...
    for col in numeric_cols:
        if col not in df_copy.columns or not pd.api.types.is_numeric_dtype(
            df_copy[col]
        ):
            logging.warning(
                f"Column '{col}' not numeric or found for ATR. " "ATR may be incorrect."
            )
...
def load_and_prepare_minute_data(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.critical(
            f"CRITICAL: Minute data file not found: '{file_path}'. Exiting."
        )
        sys.exit(1)
...
def preprocess_data():
    ...
    try:
        ...
        df_daily = pd.read_csv(
            DAILY_DATA_CSV_PATH, parse_dates=["Date"], index_col="Date"
        )
        ...
        df_daily["52w_high"] = (
            df_daily["Close"].rolling(window=TRADING_DAYS_PER_YEAR).max()
        )
        df_daily["52w_low"] = (
            df_daily["Close"].rolling(window=TRADING_DAYS_PER_YEAR).min()
        )
        ...
        df_minute_dates = df_minute.index.normalize()
        df_minute = pd.merge(
            df_minute,
            df_daily[features_to_map],
            left_on=df_minute_dates,
            right_index=True,
            how="left",
        )
    ...
    df_minute["Typical_Price_Volume"] = (
        df_minute["Volume"]
        * (df_minute["High"] + df_minute["Low"] + df_minute["Close"])
        / 3
    )
    df_minute["Cumulative_Typical_Price_Volume_Daily"] = df_minute.groupby(
        df_minute.index.date
    )["Typical_Price_Volume"].cumsum()
    df_minute["Cumulative_Volume_Daily"] = df_minute.groupby(df_minute.index.date)[
        "Volume"
    ].cumsum()
    df_minute["VWAP"] = df_minute["Cumulative_Typical_Price_Volume_Daily"] / (
        df_minute["Cumulative_Volume_Daily"] + EPSILON
    )
    ...
    annualization_30min = np.sqrt(
        TRADING_DAYS_PER_YEAR * (TRADING_MINUTES_PER_DAY / 30)
    )
    df_minute["realized_vol_5min"] = (
        df_minute["Close"].pct_change().rolling("5min").std() * annualization_5min
    )
    df_minute["realized_vol_30min"] = (
        df_minute["Close"].pct_change().rolling("30min").std() * annualization_30min
    )
    ...
    market_open_dt = (
        pd.to_datetime(df_minute.index.date)
        + pd.to_timedelta(MARKET_OPEN_HOUR, unit="h")
        + pd.to_timedelta(MARKET_OPEN_MINUTE, unit="m")
    )
    df_minute["hours_since_open"] = (
        df_minute.index - market_open_dt
    ).total_seconds() / 3600
    ...
    try:
        df_minute.drop(columns=["key_0"], inplace=True, errors="ignore")
